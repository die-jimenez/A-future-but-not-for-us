...
public static class EditorGUILayoutArrays
{
    ...
    #region Metodo hechos por el creador del script
    ...
    #region Metodos hechos por mi
    public static void ObjectArrayField<T>(string label, ref bool open, T[] array) where T : class
    {
        open = EditorGUILayout.Foldout(open, label);
        int newSize = array.Length;
        ...
        if (open)
        {
            ...
            for (var i = 0; i < newSize; i++)
            {
                //Al tratarse de una clase generica "T" hay que hacer esto en dos partes
                //(Es un dolor de cabeza de tipos de variables)
                Object obj = array[i] as Object;
                obj = EditorGUILayout.ObjectField("Value " + i, obj, typeof(T), true);
                array[i] = obj as T;
            }
        }
    }
